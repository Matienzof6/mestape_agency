        ...
        >
            <Link to={`/blog/${data.slug}`}
                onMouseEnter={()=>{
                    ...
                    title.classList.add('text-[#0073ec]')
                }} 
                onMouseLeave={()=>{
                    const img = document.getElementById(index)
                    img.classList.remove('object-fill')
                    const title = document.getElementById(`title`+data.id)
                    title.classList.remove('text-[#0073ec]')
                }}
                ...
                <div className="lg:flex min-w-0 lg:flex-1 items-center">
                  ...
                  <div className="min-w-0 flex-1 px-8 p-4 ">
                    ...
                    <div className="lg:absolute lg:top-28">
                        ...
                        <span className=" hover:text-[#0073ec]  mx-1 font-medium text-gray-800 text-sm "><Link to={`/category/${data.category.slug}`}>{data.category.name}</Link></span> <span className="text-gray-300">&middot;</span> 
