...
class ListPostsByCategoryView(APIView):
    ...
    def get(self, request, format=None):
        if Post.postobjects.all().exists():
            ...
            if not Category.objects.filter(parent=category).exists(): #Filtrar categoria sola
                posts = posts.filter(category=category)
            
            else: #Si esta categoría tiene hijos, filtrar por la categoria padre y sus hijos
...
class PostDetailView(APIView):
    ...
    def get(self, request, slug, format=None):
        if Post.postobjects.filter(slug=slug).exists():
            ...
            address = request.META.get('HTTP_X_FORWARDED_FOR') #Tomamos la info de ip del ususario
            
            if address:
                ip = address.split(',')[-1].strip() # Se corta la direccion de esta manera
            else:
                ip = request.META.get('REMOTE_ADDR') # Y si no, se agarra el address

            # Y si no existe un viewCount, y no queremos duplicados
